...
- 楽天レシピカテゴリランキングAPIから総合人気ランキングと高タンパク系ランキングを取得
- ランキングレシピにも食材一致率と不足食材を表示
...
楽天レシピカテゴリランキングAPIでは、`applicationId`と`accessKey`をクエリパラメータとして送信します。楽天アプリ設定でリファラ制限を利用する場合は、APIリクエストに`Referer`と`Origin`ヘッダーを付与します。
...
楽天レシピカテゴリランキングAPIを利用し、以下の2種類のランキングを提供します。

| 種類 | API | 内容 |
| --- | --- | --- |
| 総合人気ランキング | `/api/recipes/ranking/general` | カテゴリ未指定の総合ランキング上位5件 |
| 高タンパクランキング | `/api/recipes/ranking/protein` | 鶏むね肉、豆腐、肉カテゴリなどから重複を除いた上位5件 |

ランキング取得時は、楽天APIの公式エンドポイント`Recipe/CategoryRanking/20170426`を利用します。短時間の連続アクセスで`429 Too Many Requests`が発生しないよう、リクエスト間隔の制御と再試行を行います。

取得したランキングレシピは、通常のおすすめレシピと同じく詳細ページから材料を取得し、現在の在庫食材に対する食材一致率と不足食材を算出します。フロント画面では、総合人気ランキングと高タンパクランキングを横スクロールできるカードUIとして表示します。
